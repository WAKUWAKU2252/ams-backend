...
export const asset = pgTable(
  'asset',
  {
    ...
    /**
     * **วันที่ตั้งหนี้** — OPCH.DocDate ของใบกำกับซื้อใบแรกสุดของชิ้นนี้
     *
     * ★ ไม่ใช่ "วันที่ได้ของ" และห้ามเรียกแบบนั้น: ใบกำกับออกหลังรับของได้เป็นเดือน
     *   ส่วนวันรับของจริงอยู่ที่ใบ GRPO (OPDN/PDN1) ซึ่ง **ผูกกับสินทรัพย์ไม่ได้** —
     *   บรรทัดใบรับของอ้าง "รหัสจัดซื้อ" (ItemType 'I') ไม่ใช่เลขสินทรัพย์ (ItemType 'F')
     *   ตรวจแล้วด้วย PDN1 ของ COM-100-05-002: ไม่มีสักแถว
     *
     * ★ NULL = ชิ้นนี้ไม่เคยมีใบกำกับใน SAP ซึ่งเป็นคำตอบที่ถูก ไม่ใช่ข้อมูลขาด
     *   (เดิมถอยไปใช้ OITM.CreateDate เมื่อไม่มีใบกำกับ ทำให้คอลัมน์เดียวเก็บสองความหมาย
     *    ปนกันโดยไม่มีอะไรบอกว่าแถวไหนเป็นอันไหน — วันสร้างย้ายไป sapCreatedDate แล้ว)
     */
    acquisitionDate: date(),
    /**
     * วันที่ Finance ออกเลขสินทรัพย์ให้ใน SAP — OITM.CreateDate
     *
     * นี่คือ "ลงทะเบียนเมื่อ" ตัวจริง (คนละตัวกับ createdAt ซึ่งเป็นวันที่แถวนี้ถูก sync
     * เข้ามาใน AMS — ของเก่าที่ SAP ลงทะเบียนไว้ปี 2017 มี createdAt เป็นปี 2026 ทั้งชุด)
     *
     * ★ SAP เป็นเจ้าของ เขียนทับทุกรอบ sync เหมือน acquisitionDate — และเติมให้แถว
     *   PO_FLOW ด้วย เพราะเป็นข้อเท็จจริงฝั่ง SAP ล้วนที่ AMS ไม่มีทางรู้เอง
     *   (ต่างจาก departmentId/employeeId/locationId ที่ AMS รู้ดีกว่า ห้ามให้ SAP ทับ)
     * ★ NULL = ชิ้นนั้นยังไม่มีแถวใน OITM (ของที่ลงทะเบียนผ่าน AMS แต่ยังไม่ได้ออกเลข)
     */
    sapCreatedDate: date(),
    ...
    // ── ตำแหน่งบนผังชั้น (เพิ่มใน 0007, เปลี่ยนกรอบอ้างอิงใน 0022) — จุดที่ผู้ใช้ปักหมุด
    //    ว่าของชิ้นนี้วางตรงไหน
    //
    // ⚠️ 0022 เปลี่ยนความหมายของสองช่องนี้: เดิมเป็นสัดส่วนของ "ผังรูปละห้อง"
    //    ตอนนี้เป็นสัดส่วนของ "ผังแผ่นใหญ่ทั้งชั้น" ใบเดียวกับที่ asset_sub_location.polygon
    //    ใช้ (ไฟล์ = asset_sub_location.planKey) ตัวเลขหน้าตาเหมือนเดิมแต่คนละกรอบ
    //    ตอนเปลี่ยนไม่มีข้อมูลเก่าให้แปลง (ยังไม่มีใครปักหมุด) — ถ้าเจอ posX/posY ที่ลงก่อน
    //    0022 แปลว่ามาจากทางที่ไม่ควรมี ต้องปักใหม่ ไม่ใช่แปลงพิกัด
    //
  ...
  (table) => [
    ...
    // ── แผนกต้องเป็นของบริษัทเดียวกับชิ้นนี้ (0026) ───────────────────────────
    //
    // composite FK ไม่ใช่ FK คอลัมน์เดียว — ครอบ departmentId ไปในตัวอยู่แล้ว จึงไม่ต้องมี
    // สองตัว (แพทเทิร์นเดียวกับ fk_asset_grpo_line และ employee_company)
    //
    // ★ ทำไมต้องมี: ก่อนหน้านี้ไม่มีอะไรบังคับให้ asset.companyCode กับ department.companyCode
    //   ตรงกัน ของ UBP จึงชี้แผนกของ UBA ได้เงียบ ๆ ผลที่ตามมาไม่ใช่แค่ข้อมูลปนกัน:
    //   ตารางสรุปรายแผนกบน dashboard ต้องเลือกระหว่าง "กรองบริษัทให้ถูก" กับ "ยอดรวม
    //   กระทบกับ totals" อย่างใดอย่างหนึ่ง (เจอจริงตอนทำ filter — ดู summarizeByDepartment
    //   ที่ต้องเขียน OR เผื่อกรณีนี้ไว้) และรายงานรายแผนกของแต่ละบริษัทจะนับของบริษัทอื่นปน
    //
    // ★ ยังไม่ cascade เหมือนเดิม — ลบแผนกที่มีสินทรัพย์สังกัดอยู่ไม่ได้ ต้องปิด isActive
    //
    // departmentId เป็น NULL ได้ (ของที่ยังไม่ระบุแผนก) — pg ใช้ MATCH SIMPLE เป็นค่าตั้งต้น
    // ซึ่งปล่อยผ่านเมื่อคอลัมน์ใดคอลัมน์หนึ่งเป็น NULL จึงไม่บล็อกกลุ่มนั้น
    foreignKey({
