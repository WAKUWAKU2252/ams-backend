...
import { attachment, grpoInvoice, asset } from '@intrastucture/db/schema';
...
//
// ใช้ satisfies ไม่ใช่ `: Record<DocType, UploadRule>` โดยตั้งใจ — satisfies ตรวจว่าครบ
// ทุก DocType และรูปถูกต้อง แต่ยังคง literal key ของ ext ไว้ ทำให้ rule.ext[mime]
// ได้ string ไม่ใช่ string | undefined หลังผ่าน `in` guard แล้ว
const RULES = {
  INVOICE: {
...
} satisfies Record<DocType, UploadRule>;
...
export async function getFileOrFail(id: string): Promise<StoredFileRef> {
...
export async function softDelete(id: string, deletedBy: number): Promise<DeleteAttachmentResult> {
  const [row] = await db
    .update(attachment)
  ...
  // ตรงนี้ "ไม่เจอ" เป็นผลลัพธ์ปกติ (ลบซ้ำ) ไม่ใช่ผิดสัญญา จึงเช็คเองแล้วโยน 404
  // ไม่ใช้ requireRow ที่ตั้งใจไว้สำหรับกรณีที่ต้องมีแถวเสมอ
  if (!row) throw new NotFoundError(`Attachment ${id}`);
  return { success: true };
}
