...
import { asset, assetRequest, assetRequestLine, grpoLine } from '@intrastucture/db/schema';
...
export async function assertEditableBy(requestId: number, userId: number) {
  ...
  if (!presence.isHolder('draft', requestId, userId)) {
...
async function countRegistered(grpoLineId: string): Promise<number> {
  return requireScalar(
    await db
      .select({ value: count() })
      .from(asset)
      .where(and(eq(asset.grpoLineId, grpoLineId), isNull(asset.deletedAt))),
    `count asset ของรอบรับของ ${grpoLineId}`,
  );
}
...
export async function declareLine(
  ...
  // เทียบกับค่าที่ปัดขึ้นแล้ว ไม่ใช่ค่าดิบ: receivedQty เป็น numeric ได้ (เช่น 2.5) แต่ declaredQty
  // เป็นจำนวนเต็มเสมอ ถ้าเทียบตรง ๆ จะไม่มีวันเท่ากัน ทางลัด "ไม่ได้แตกรายการ" ก็ตายสนิท
  // แล้วทุกการแจ้งจะสร้างแถวค้างไว้พร้อม badge ทั้งที่จำนวนช่องเท่าเดิม
  if (declaredQty === Math.ceil(line.receivedQty)) {
    const deleted = await db
      .delete(assetRequestLine)
      .where(and(eq(assetRequestLine.requestId, requestId), eq(assetRequestLine.grpoLineId, grpoLineId)))
      .returning();
    // แตะ updatedAt + บอกคนอื่น เฉพาะตอนมีการเปลี่ยนจริง — ไม่มีอะไรถูกลบ = ไม่มีเหตุให้ใครโหลดใหม่
    if (deleted.length > 0) {
      await touchRequest(requestId);
      presence.notifyStatus({ requestId, action: 'line-reverted', requestStatus: req.status, assetId: null, actorId: userId });
    }
  ...
  await touchRequest(requestId);
  // จำนวนช่องของทั้งรอบเปลี่ยน — เป็นเรื่องระดับรอบ ไม่ใช่ของชิ้นใดชิ้นหนึ่ง (assetId = null)
  presence.notifyStatus({ requestId, action: 'line-declared', requestStatus: req.status, assetId: null, actorId: userId });
  return row;
}
...
export async function removeDeclaredLine(requestId: number, grpoLineId: string, userId: number) {
  ...
  const registered = await countRegistered(grpoLineId);
  // ปัดขึ้นให้ตรงกับจำนวนช่องที่ฟอร์มเปิดให้จริง ไม่งั้นชิ้นสุดท้ายของบรรทัดที่รับมาเป็นเศษ
  // (เช่น 2.5 → 3 ช่อง) จะทำให้ยกเลิกการแจ้งไม่ได้ทั้งที่ลงทะเบียนตามที่ระบบอนุญาต
  if (registered > Math.ceil(line.receivedQty)) {
    throw new BadRequestError(
